...
const mixedDossier = ctx.scopeDossier([c.rows[405], c.rows[406], c.rows[0]]);
assert.match(mixedDossier, /current catalogue lots<\/span><span class="v">2<\/span>/);
assert.match(mixedDossier, /present at the last check<\/span><span class="v">1<\/span>/);
assert.match(mixedDossier, /availability unverified<\/span><span class="v">1<\/span>/);
assert.match(mixedDossier, /missing or archived records<\/span><span class="v">1<\/span>/);
