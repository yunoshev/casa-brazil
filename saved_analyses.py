...
def load_saved_analyses(path, source):
    ...
    for lot_id, report in data["reports"].items():
        if not re.fullmatch(r"[a-f0-9]{16}", lot_id) or set(report) != {
            "source_url", "document_sha256", "analyzed_at", "language", "analysis"
        }:
            raise ValueError("invalid saved analysis fields")
        if report["language"] != "pt" or not re.fullmatch(r"[a-f0-9]{64}", report["document_sha256"]):
        ...
        a = report["analysis"]
        if set(a) != {"contract", "document_type", "identity", "entries", "summary", "warnings", "confidence", "disclaimer"}:
            raise ValueError("invalid saved analysis contract")
        if a["contract"] != "brazil_matricula_v1" or a["document_type"] != "matricula":
        ...
        def safe(value, maximum=3000):
            return isinstance(value, str) and 0 < len(value) <= maximum and not re.search(r"[<>\x00-\x08]", value)
        if not safe(a["summary"]) or not safe(a["disclaimer"]):
            raise ValueError("invalid analysis text")
        if not isinstance(a["warnings"], list) or not 1 <= len(a["warnings"]) <= 20 or not all(safe(w) for w in a["warnings"]):
            raise ValueError("invalid warnings")
        for entry in a["entries"]:
            if set(entry) != {"kind", "number", "title", "summary", "effect", "citations"} or entry["kind"] not in {"R", "AV"} or type(entry["number"]) is not int or entry["effect"] != "unclear":
            ...
            for citation in entry["citations"]:
                if set(citation) != {"page", "quote"} or type(citation["page"]) is not int or not 1 <= citation["page"] <= 150 or not safe(citation["quote"], 360):
                    raise ValueError("invalid citation")
        row = rows.get(lot_id)
        if row is not None and row[cols["src"]] == "caixa" and row[cols["link"]] == report["source_url"]:
